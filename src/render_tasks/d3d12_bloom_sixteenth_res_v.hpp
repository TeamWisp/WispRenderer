...
namespace wr
{
	...
	namespace internal
	{
		...
		template<typename T>
		inline void SetupBloomSixteenthVTask(RenderSystem& rs, FrameGraph& fg, RenderTaskHandle handle, bool resize)
		{
			auto& n_render_system = static_cast<D3D12RenderSystem&>(rs);
			auto& data = fg.GetData<BloomSixteenthVData>(handle);
		...
		template<typename T>
		inline void ExecuteBloomSixteenthVTask(RenderSystem& rs, FrameGraph& fg, SceneGraph& sg, RenderTaskHandle handle)
		{
			auto& n_render_system = static_cast<D3D12RenderSystem&>(rs);
		...
		inline void DestroyBloomSixteenthVTask(FrameGraph& fg, RenderTaskHandle handle, bool resize)
		{
			if (!resize)
			{
				auto& data = fg.GetData<BloomSixteenthVData>(handle);
	...
	template<typename T>
	inline void AddBloomSixteenthVTask(FrameGraph& frame_graph)
	{
		RenderTargetProperties rt_properties
		{
			...
			RenderTargetProperties::ResolutionScalar(0.0625f)
		};

		RenderTaskDesc desc; 
		desc.m_setup_func = [](RenderSystem& rs, FrameGraph& fg, RenderTaskHandle handle, bool resize) {
			internal::SetupBloomSixteenthVTask<T>(rs, fg, handle, resize);
		};
		desc.m_execute_func = [](RenderSystem& rs, FrameGraph& fg, SceneGraph& sg, RenderTaskHandle handle) {
			internal::ExecuteBloomSixteenthVTask<T>(rs, fg, sg, handle);
		};
		desc.m_destroy_func = [](FrameGraph& fg, RenderTaskHandle handle, bool resize) {
			internal::DestroyBloomSixteenthVTask(fg, handle, resize);
		};
		desc.m_properties = rt_properties;
		desc.m_type = RenderTaskType::COMPUTE;
		desc.m_allow_multithreading = true;

		frame_graph.AddTask<BloomSixteenthVData>(desc, L"Bloom Quarter res vertical Blur");
	}
