	...
	namespace internal
	{
		...
		inline void SetupDoFCompositionTask(RenderSystem& rs, FrameGraph& fg, RenderTaskHandle handle, bool resize)
		{
			...
			for (auto frame_idx = 0; frame_idx < versions; frame_idx++)
			{
				...
				// Bokeh
				{
					auto cpu_handle = data.out_allocation.GetDescriptorHandle(COMPILATION_EVAL(rs_layout::GetHeapLoc(params::dof_composition, params::DoFCompositionE::BOKEH)));
					d3d12::CreateSRVFromSpecificRTV(source_rt_bokeh_filtered, cpu_handle, frame_idx, source_rt_bokeh_filtered->m_create_info.m_rtv_formats[frame_idx]);
				}
				// Cone of confusion
				{
