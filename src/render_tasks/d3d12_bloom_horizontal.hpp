...
namespace wr
{
	...
	namespace internal
	{
		...
		inline void SetupBloomHorizontalTask(RenderSystem& rs, FrameGraph& fg, RenderTaskHandle handle, bool resize)
		{
			...
			data.out_pipeline = ((d3d12::PipelineState*)ps_registry.Find(pipelines::bloom_h));
		...
		inline void DestroyBloomHorizontalTask(FrameGraph& fg, RenderTaskHandle handle, bool resize)
		{
			if (!resize)
			{
				auto& data = fg.GetData<BloomHData>(handle);
	...
	inline void AddBloomHorizontalTask(FrameGraph& frame_graph)
	{
		...
		desc.m_destroy_func = [](FrameGraph& fg, RenderTaskHandle handle, bool resize) {
			internal::DestroyBloomHorizontalTask(fg, handle, resize);
		};
		desc.m_properties = rt_properties;
		desc.m_type = RenderTaskType::COMPUTE;
		desc.m_allow_multithreading = true;

		frame_graph.AddTask<BloomHData>(desc, L"Bloom Horizontal Blur");
	}
