	...
	namespace internal
	{
		...
		inline void SetupRTHybridTask(RenderSystem & render_system, FrameGraph & fg, RenderTaskHandle & handle)
		{
			...
			for (int frame_idx = 0; frame_idx < 1; ++frame_idx)
			{
				...
				cpu_handle = d3d12::GetCPUHandle(as_build_data.out_rt_heap, frame_idx, COMPILATION_EVAL(rs_layout::GetHeapLoc(srv::full_raytracing, srv::RTHybridE::GBUFFERS)));
		...
		inline void ExecuteRTHybridTask(RenderSystem & render_system, FrameGraph & fg, SceneGraph & scene_graph, RenderTaskHandle & handle)
		{
			...
			if (n_render_system.m_render_window.has_value())
			{
				...
				auto cpu_handle = d3d12::GetCPUHandle(as_build_data.out_rt_heap, 0, COMPILATION_EVAL(rs_layout::GetHeapLoc(srv::full_raytracing, srv::RTHybridE::LIGHTS)));
				...
				if (scene_graph.m_skybox.has_value())
				{
					...
					auto cpu_handle = d3d12::GetCPUHandle(as_build_data.out_rt_heap, 0, COMPILATION_EVAL(rs_layout::GetHeapLoc(srv::full_raytracing, srv::RTHybridE::SKYBOX))); // here
