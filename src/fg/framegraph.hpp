...
{
public:
  explicit framegraph(wr::RenderSystem& render_system) : render_system_(render_system) {}
  ...
  render_task<data_type>*                  add_render_task      (argument_types&&... arguments)
  {
    ...
    render_task->setup(render_system_, builder);
  ...
  void                                     execute              () const
  {
    for(auto& step : timeline_)
    {
      ...
      step.render_task->execute(render_system_);
...
protected:
  ...
  wr::RenderSystem& render_system_;
};
