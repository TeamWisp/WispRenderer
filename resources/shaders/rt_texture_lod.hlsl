...
float2 WorldToScreen(float3 pos, float4x4 view)
{
	float4x4 ivp = inverse(view);
	float4 p = mul(float4(pos, 0), ivp);
...
float ComputeTextureCoordsArea(float2 P0, float2 P1, float2 P2, float2 UV0, float2 UV1, float2 UV2, Texture2D T)
{
	float w, h;
	T.GetDimensions(w, h);
	float ta = w * h * length((UV1.x - UV0.x) * (UV2.y - UV0.y) - (UV2.x - UV0.x) * (UV1.y - UV0.y));
}
