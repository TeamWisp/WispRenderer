...
float3 shade_light(float3 pos, float3 V, float3 albedo, float3 normal, float metallic, float roughness, Light light, inout uint rand_seed, uint shadow_sample_count, uint depth)
{
	uint tid = light.tid & 3;
	...
	float shadow_factor = GetShadowFactor(wpos, L, t_max, shadow_sample_count, depth, rand_seed);
...
float3 shade_pixel(float3 pos, float3 V, float3 albedo, float metallic, float roughness, float3 emissive, float3 normal, inout uint rand_seed, uint shadow_sample_count, uint depth)
{
	uint light_count = lights[0].tid >> 2;	//Light count is stored in 30 upper-bits of first light
	...
	for (uint i = 0; i < light_count; i++)
	{
		res += shade_light(pos, V, albedo, normal, metallic, roughness, lights[i], rand_seed, shadow_sample_count, depth);
	}

	return res + emissive;
}

float4 DoShadowAllLights(float3 wpos, float3 V, float3 normal, float metallic, float roughness, float3 albedo, uint shadow_sample_count, uint depth, inout float rand_seed)
{
	uint light_count = lights[0].tid >> 2;	//Light count is stored in 30 upper-bits of first light
	...
	for (uint i = 0; i < light_count; i++)
	{
		...
		float shadow = GetShadowFactor(wpos, L, t_max, shadow_sample_count, depth, rand_seed);
