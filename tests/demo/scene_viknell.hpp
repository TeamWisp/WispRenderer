...
namespace viknell_scene
{
	namespace resources
	{
		...
		void CreateResources(wr::RenderSystem* render_system)
		{
			...
			test_model = model_pool->LoadWithMaterials<wr::VertexColor>(material_pool.get(), texture_pool.get(), "resources/models/sponza/sponza.obj");
	...
	void CreateScene(wr::SceneGraph* scene_graph, wr::Window* window)
	{
		...
		// Lights
		auto point_light_0 = scene_graph->CreateChild<wr::LightNode>(nullptr, wr::LightType::POINT, DirectX::XMVECTOR{ 2, 2, 2 });
		point_light_0->SetRotation({ 20.950f, 0.98f, 0.f });
		point_light_0->SetPosition({ -0, 2, 0 });
		point_light_0->SetRadius(10000);
